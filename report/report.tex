...
Like can be seen in Fig. \ref{fig:scrongscaling-20} all implementation have an improved runtime as the MPI processes
increases up and including 4 processes. All vertex distribution implementation perform similarly apart from the
supervertices pointer jumping one which performs worse in comparison as the number of the MPI processes increases. The
edge distribution implementations also perform similarly with the best performing algorithm being Edge Distributed union
find w. reduced edges, running on 32 MPI processes. The Edge Distribution implementations scale better and the runtime
almost halves as the number of MPI processes doubles up until 4 MPI processes when there start to be diminishing
returns. It should be noted that the ratio between vertices and edges is only 1:16 and therefore seeing diminishing
returns past 16 MPI processes is not surprising and more dense graphs might benefit from running on a higher number of
MPI processes.

\mypar{How well do our algorithms perform compared to the baseline}
To answer the question we use the strong scaling study results including the those for the parallel boost
implementation.
...
From Fig. \ref{fig:scrongscaling-19}, it can be seen that all our implementations exceed the performance of the parallel
boost implementation when work is distributed on 8 or less processes and all the edge distribution algorithms outperform
the baseline for all measured cases. In Table. \ref{tab:scrongscaling-19-table} we compare our best performing
implementation, Edge distributed union find w. reduced edges against the baseline, ParallelBoost. Our implementation
performs best when using 32 processes, which compared to ParallelBoost results in $2.4329s/0.1085s \approx 22.4$ x
speedup.
