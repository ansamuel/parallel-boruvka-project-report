...
\begin{table*}
  \begin{tabularx}{\linewidth}{ l | X  X  X  X  X  X  X }
    \toprule
    \textbf{No. processes} & \textbf{1} & \textbf{2} & \textbf{4} & \textbf{8} & \textbf{16} & \textbf{32} & \textbf{64} \\\midrule
    Edge distributed union find w. reduced edges & 0.5050 & 0.2997 & 0.2001 & 0.1254 & 0.11 & 0.1085 & 0.1094 \\ 
    ParallelBoost & 13.0532 & 9.4513 & 5.3956 & 3.4787 & 2.6483 & 2.4329 & 2.4421 \\ \bottomrule \hline
  \end{tabularx}
...
\begin{figure}
  ...
  \caption{Comparison of time spent on parts. Vertex count $2^{19}$, MPI processes 64.}
  \label{fig:comparison}
\end{figure}

\mypar{Detailed Runtime Analysis}
Fig. \ref{fig:comparison} gives us more details as to why the edge distributed implementations outperform the vertex
distributed implementations. The work distribution is a huge bottleneck for the vertex distributed implementations,
whereas it only makes up a small part of the total runtime of the edge distribution implementations. An explanation of
this could be that the edge distribution implementations use a scatter for their work distribution, potentially using a
tree appraoch to achieve this. In the vertex distribution implementations the work is distributed by the root MPI
process one after the other, giving us a worse theoretical runtime. The lightest edge selection performs very similar
accross all implementations. On the other hand, the union find merging strategy outperforms all others, except for the
iterative vertex fixing approach. Interestingly, this approach outperforms the other two pointer jumping strategies,
even though it has a worse theoretical runtime. This result indicates that the additional synchronization required in
these strategies is not worth the tighter theoretical runtime bound. The terrible runtime of the supervertex pointer
jumping merge strategy gives a reason as to why it performed so much worse compared to the other pointer jumping
approaches in the strong scaling study. %TODO, maybe rather stress that the supervertex one has the most synchronization needed, which gives an explanation of why it performs the worst, thus the strongscaling results.
In addition, the graph shows that a big portion of the standard deviation of the
vertex distributed supervertices pointer jumping implementation comes from the merge strategy. This strengthens the
