...
Randomly generated Kronecker graphs are used to evaluate the performance of our implementation. In this paper we present a parallel MST implementation which performs significantly better than the state-of-the-art implementation.
...
More on this in section \ref{sec:exp}. % TODO maybe update reference to a more specific one later on (Mike)
...
% TODO is this argumentation valid? (Mike)
...
required is not worth the tighter theoretical runtime bound. The same reasoning holds for why the supervertex pointer jumping
merge strategy, the strategy requiring most amount of synchronization, performed so terribly in the strong scaling study.
...
As is evident form the benchmark evaluation % TODO: Reference (Mike)
...
the worst. In conclusion, we can say that we achieved our goal and found approaches that perform better than a state-of-the-art implementation. The Parallel BGL is, of course, more sophisticated and supports generic graph data structures as
