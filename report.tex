...
\begin{itemize}
  \item Edge distribution with Union Find
  \item Edge distribution with Union Find and Reduced Edges
  \item Vertex distribution with Simple Root Iteration
  \item Vertex distribution with Pointer Jumping
  \item Vertex distribution with Supervertex Pointer Jumping
  \item Vertex distribution with Union Find
\end{itemize}
...
selected, by any computation unit, to the resulting MST. For that reason, the result is a spanning tree or a forest for a non-connected initial graph. The computed
