...
The benchmarking were run on the Raclette cluster, setup and maintained by a group of students from ETH.
...
Like mentioned in Approach %TODO add reference
the benchmarking was done on Kronecker Graphs of different size but the number of edges is always 16 times the number of vertices. The baseline which the results are compared against is the parallel version of Boost.

\mypar{Strong scaling}
The first question is how well do our algorithms scale with increasing number of computing units when the input is a Kronecker graph with number of vertices being $2^{19}$ and number of edges being $2^{19}*16$.

$<Insert Ploth>$
%strongscaling, full algos, size 19 -> line plot

Like can be seen in the plot %TODO add reference
all our implementation exceed the performance of the parallel boost implementation. The best performing one being the algorithm using Edge Distribution with edge reduction and Union Find running on 64 computing units. %TODO add how much better 
Reducing the edges did reduce the runtime but only marginally, since the benefits only kick in from 3rd iterations on but the first iterations are the most expensive ones. Vertex distribution did not seem to pay off for this kind of input graph. The supervertex Pointer Jumping method did not pay off either, probably because of a lot of synchronization needed within each recursion.
For the edge distribution Union Find, since the ratio between the number of edges and vertices is 16:1 there are diminishing returns around 16 computing units. This is because when the edges are divided between 16 computing units the work needed to find the lightest adjacent edge becomes linear in the number of nodes. The union find, which is done locally and also runs linearly in the number of nodes, slowly becomes the bottleneck. Past 64 computing units the time actually starts to increase since at that point the overhead of distributing the work to more computing units stops paying off.

\mypar{Weakscaling}
How well do our algorithms scale with increasing graph sizes?

$<Insert Ploth>$
%Weakscaling, full algos, sizes 11-19, line plot
% TODO if not written like that in plot, make it clear that size X means vertex number being 2^10
...
For the edge distribution we can see that the merging part or the Union Find takes up around half of the time??, becoming one of the bottleneck along with the synchronization in the finding the lightest adjacent edge part.
For the vertex distribution it seems that expensive pre-processing for distributing edges takes up a big chunk of the total runtime. Additionally, the fact that even though initially computing units containing all edges for their respective vertices, as vertices are merged, reduction is needed to find the lightest adjacent edge for a connected component.
