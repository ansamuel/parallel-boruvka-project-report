...
% TODO add intro text or indentation here to be consistent with report style
\mypar{Hardware}
...
by a group of students from ETH. The cluster consists of four identical nodes, each of which has two 64-core AMD EPYC
7742 CPUs running at 2.25GHz with 512GB DDR4 3200MT/s RAM and four Nvidia Tesla V100 GPUs with 32GB GDDR5 VRAM for a
total of 512 cores, 2TB of memory and 16 GPUs. The four nodes are connected using a speedy interconnect from Mellanox,
namely HDR ConnectX-6 adapters for a 200Gbit/s connection.
...
% programs you ran.
As described in section, % TODO reference kronecker graph generator
all benchmarks were performed using Kronecker graphs. The amount of edges is in all cases $16$ times the amount of
vertices. It is also important to mention that the implementation edge distribution using pointer jumping gave
unbelievably bad results. Since we are not certain about the correctness of this implementation and since there was not
enough time to look into the nature of these strange results, we decided to not include them in our reported
measurements.
