...
The first question is how well do our algorithms scale with increasing number of computing units when the input is a Kronecker graph with number of vertices being $2^{19}$ and number of edges being $2^{19}*16$ and how does it compare against the baseline.

$<Insert Ploth>$
%strongscaling, full algos, size 19 -> line plot
% TODO write important information not mentioned in plot, like number of vertives in graph being 2^19

Like can be seen in the plot %TODO add reference
all our implementation exceed the performance of the parallel boost implementation when work is distributed on less than X computing units. The best performing one being the algorithm using Edge Distribution with edge reduction and Union Find running on 64 computing units. %TODO add how much better 
Reducing the edges did reduce the runtime but only marginally, since the benefits only kick in from 3rd iterations on but the first iterations are the most expensive ones. Vertex distribution was slower and the supervertex Pointer Jumping method also slower than all other merging mechanisms. That may partly be  because of a lot of synchronization needed within each recursion.
...
$<Insert Ploth>$
%Weakscaling, full algos, sizes 11-19, line plot

%TODO add some analyziz of the plot results

\mypar{Profiling}
Which part of the algorithms are taking up most of the time ?
