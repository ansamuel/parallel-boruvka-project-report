...
All four nodes were allocated for all benchmark executions. In addition, we pinned the CPU cores, such that the amount
of used cores was equal on all computation nodes. We decided on this allocation strategy to get more consistent
measurements. Otherwise, benchmarks with a low core count would have had an unfair advantage, as they would have
required less network communication, resulting in potential spikes at threasholds where an additional node is needed.
...
% programs you ran.
As described in section \ref{par:kronecker_graph_generator}, all benchmarks were performed using Kronecker graphs. The
amount of edges is in all cases $16$ times the amount of vertices. It is also important to mention that the
implementation edge distribution using pointer jumping gave unbelievably bad results. Since we are not certain about the
correctness of this implementation and since there was not enough time to look into the nature of these strange results,
we decided to not include them in our reported measurements.

\mypar{Baseline Comparison}
To compare our implementations with the baseline, we performed a strong scaling study using graphs with vertex count
$2^{19}$. The number of used MPI processes in this strong scaling study were all powers of $2$ up until $2^6$. The
baseline implementation did not allow us to go any furhter than that, as the cluster did not finish the execution. We
were not able to determine the reason for this issue.

\mypar{Implementation Comparison}
To compare our implementations with one another, we performed a strong and weak scaling study. The strong scaling study
was performed using graphs with vertex count $2^{20}$. As this time there was no limitation due to the baseline not
working with higher amounts of MPI processes, the number of used MPI processes was all powers of $2$ up until the
maximum of $2^9$. The weak scaling study was performed using $1$ MPI node using a graph of size $2^{15}$, $4$ MPI nodes
using a graph of size $2^{16}$, $16$ MPI nodes using a graph of size $2^{17}$, $64$ MPI nodes using a graph of size
$2^{18}$ and $256$ MPI nodes using a graph of size $2^{19}$.

\mypar{Detailed Runtime Analysis}
To get a better understanding about which parts of the algorithms require the most amount of runtime, we performed more
fine-grained measurements. We differentiated between graph distribution, lightest edge selection and merging.
...
algorithm iterations, another arithmetic mean was computed, using the previousely mentioned results as its input.
Although taking the maximum or minimum accross all MPI processes could have also been possible, the difference between
these and the mean were negligibly small, as all algorithms call a synchronizing subroutine right before termination.

\mypar{Repetitions}
Due to problems with crashing nodes and other people wanting access to the cluster, we tried to achive a $90\%$
confidence interval. As such, we collected measurements until the $90\%$ confidence interval was within $5\%$ of our
reported means. This was achieved by performing $10$ repetitions in all benchmarks, whilst the first execution is
discarded and counted as warmup. The warmup is also not used to compute the measurements reported in our plots.
...
Besides the obvious step of finding new strategies and implement more combinations, a possible way on how to improve the
current implementations is by resolving the limitations described in section, % TODO ref to impl. section
i.e. storing and returning the actual MST and allow any input graph to be executed on any number of MPI nodes. Another
possibility would be to take more fine-grained measurements of the individual algorithm parts and try to find
optimizations based on their outcome.
