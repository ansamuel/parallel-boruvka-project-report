...
The benchmarking was run on the Raclette cluster which is set up and maintained by a group of students from ETH.
...
Like mentioned in \ref{sec:implementations}, the benchmarking was done on Kronecker Graphs of different sizes but the number of edges is always 16 times the number of vertices. The baseline which the results are compared against is the parallel version of Boost. %TODO mention the CI

\mypar{Strong scaling}
The first question is how well do our algorithms scale with increasing number of computing units. This was done with the input beign a Kronecker graph with number of vertices $2^{20}$ and number of edges being $2^{20}*16$

$<Insert Ploth>$
%strongscaling, full algos, size 20 -> line plot

The best performing algorithm is the one with Edge Distribution with edge reduction and Union Find running on 64 computing units. Reducing the edges did reduce the runtime but only marginally, since the benefits only kick in from 3rd iterations on but the first iterations are the most expensive ones. Vertex distribution was slower and the supervertex Pointer Jumping method also slower than all other merging mechanisms. That may partly be  because of a lot of synchronization needed within each recursion.
For the edge distribution Union Find, there are diminishing returns around 16 computing units. This is likely due to the fact that the ratio between the number of edges and vertices is 16:1 and when the edges are divided between 16 computing units, the work needed to find the lightest adjacent edge becomes linear in the number of nodes. The union find, which is done locally and also runs linearly in the number of nodes, slowly becomes the bottleneck. Past 64 computing units the time actually starts to increase. %TODO write about why? Could be more overhead of synchronization and distributing the work with diminishing returns

\mypar{Comparison with baseline}
How well do our algorithms perform in comparison against the baseline or parallel boost. This was done with strong scaling or with increasing number of computing units when the input is a Kronecker graph with number of vertices being $2^{19}$ and number of edges being $2^{19}*16$

$<Insert Ploth>$
%Strong scaling, skip boost, size 20 -> line plot

Like can be seen in the plot %TODO add reference
all our implementation exceed the performance of the parallel boost implementation when work is distributed on less than X computing units and all the edge distribution algorithms outperforming the baseline for these measurements. %TODO add how much better the best performing one is

\mypar{Weakscaling}
How well do our algorithms scale with increasing graph size?

$<Insert Ploth>$
%Weak scaling, full algos, sizes 15-19 -> line plot

%TODO add some analyziz of the plot results

\mypar{Profiling}
Which part of the algorithms is taking up most of the time ?

$<Insert Plot>$
%Comparison, segments, size 19 -> bar chart

For the edge distribution we can see that the merging part or the Union Find takes up XXX of the time, becoming one of the bottleneck along with the synchronization in the finding the lightest adjacent edge part??
For the vertex distribution it seems that expensive pre-processing for distributing edges takes up a big chunk of the total runtime. Additionally, the fact that even though initially computing units containing all edges for their respective vertices, as vertices are merged, edges for a connected component are distributed across different computation unit and therefore a reduction is needed for finding the lightest adjacent edge.
% TODO do we want to mention that the edges could have been sent a long to the computing unit responsible for a given root vertex. Not trivial if that would pay off or not.
