...
To answer the question we used strong scaling study results without the baseline.
...
Like can be seen in Fig. \ref{scrongscaling-20} all implementation have an improved runtime as the MPI processes increases up and including 4 processes.
 All vertex distribution implementation perform similarly apart from the supervertices pointer jumping one which performs worse in comparison as the number of the MPI processes increases. The edge distribution implementations also perform similarly with the best performing algorithm being Edge Distributed union find w. reduced edges, running on 32 MPI processes. The Edge Distribution implementations scale better and the runtime almost halves as the number of MPI processes doubles up until 4 MPI processes when there start to be diminishing returns. It should be noted that the ratio between vertices and edges is only 1:16 and therefore seeing diminishing returns past 16 MPI processes not surprising and more dense graphs might benefit from running on a higher number of MPI processes.

\mypar{How well do our algorithms perform compared to the baseline}
To answer the question we used the strong scaling study results with the results for the parallel boost implementation.
...
To answer this we use the fine-grained measurements that separates the runtimes between graph distribution, lightest edge selection and merging.
...
For vertex distribution it is obvious from Fig. \ref{comparison} that distributing the graph is a bottleneck while for the edge distributions that's a small part of the total runtime. Distributing the edges by vertices also doesn't result in much benefit for the lightest edge selection. This is probably because after merging, the edges for connected component end up being spread over different computation units. For the different merging strategy the sequential union is significantly faster than all the pointer jumping versions. Surprisingly the simple pointer jumping being the fastest of them, possibly due to fewer synchronizations needed. The union find, even though running sequentially also only takes up small part of the runtime.
